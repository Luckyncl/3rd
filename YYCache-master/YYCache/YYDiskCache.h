 ...
 * It can automatically decide the storage type (sqlite/file) for each object to get
 ...
 iOS system to get 2x~4x speed up.
    您可以编译最新版本的sqlite并忽略libsqlite3.dylib
        iOS系统加速2x~4x
 */
...
/**
 ...
 default file name.
    当需要将对象保存为文件时，将调用此块以生成
    指定键的文件名。 如果块为nil，则缓存使用md5（key）as
    默认文件名
 The default value is nil.
 */
...
/**
 ...
 在保存对象之前，可以将任何扩展数据设置为对象
   到磁盘缓存。 扩展数据也将与此对象一起保存。 你可以得到
   稍后使用“getExtendedDataFromObject：”扩展数据。
