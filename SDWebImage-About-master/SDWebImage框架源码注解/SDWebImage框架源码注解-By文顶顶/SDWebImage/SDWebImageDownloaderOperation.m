...
/**
 *  是否正在执行      (自定义的 operation 自己设置了两种状态)
 */
...
// This is set if we're using not using an injected NSURLSession. We're responsible of invalidating（无效的） this one
@property (strong, nonatomic) NSURLSession *ownedSession;

@property (strong, nonatomic, readwrite) NSURLSessionTask *dataTask;

//线程对象           注意这里是原子性的，也就是说是线程安全的
...
@implementation SDWebImageDownloaderOperation
{
    ...
    BOOL responseFromCached;         //  是否从缓存获取响应内容
}
...
// 重写 setter方法  在setter方法里面包装kvc方法
...
didReceiveResponse:(NSURLResponse *)response
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler {
    ...
    if (![response respondsToSelector:@selector(statusCode)] || ([((NSHTTPURLResponse *)response) statusCode] < 400 && [((NSHTTPURLResponse *)response) statusCode] != 304)) {
        ...
        if (self.progressBlock) {
            self.progressBlock(0, expected); // 收到响应先传递进度0 以及传递总大小
        }
        ...
        //在304的情况下，我们只需要取消操作，并从缓存返回缓存的图像。
        /**
         *  关于服务器 304状态码的解释
...
//                当我们绘制核心图形时，我们丢失了方向信息，
//                这意味着下面的图像initwithcgiimage将诞生
//                有时错误导向。（不同的图像initwithdata出生
//                在didcompletewitherror。）所以它保存在这里，通过它在以后。
