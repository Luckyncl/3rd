...
//    dispatch_barrier_sync(<#dispatch_queue_t queue#>, <#^(void)block#>)
    dispatch_barrier_sync(self.barrierQueue, ^{
        ...
        if (progressBlock) callbacks[kProgressCallbackKey] = [progressBlock copy]; // 存储下载进度block
        if (completedBlock) callbacks[kCompletedCallbackKey] = [completedBlock copy];// 存储完成进度blcok
