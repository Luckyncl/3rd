...
#pragma mark - Core types   核心的使用类型
...
/*!
 * Mulichannel input callback table 输入毁掉列表
 */
typedef struct {
...
static void serviceAudioInput(__unsafe_unretained AEAudioController * THIS, const AudioTimeStamp *outputBusTimeStamp, const AudioTimeStamp *inputBusTimeStamp, UInt32 inNumberFrames) {
    ...
    // Render audio into buffer  渲染音频到缓冲器
    if ( !useAudiobusReceiverPort ) {
...
- (id)initWithAudioDescription:(AudioStreamBasicDescription)audioDescription options:(AEAudioControllerOptions)options {
    ...
    _allowMixingWithOtherApps = options & AEAudioControllerOptionAllowMixingWithOtherApps; // 是否允许混音其他的app
    _enableBluetoothInput = options & AEAudioControllerOptionEnableBluetoothInput;  // 是否允许蓝牙输入
    ...
    _inputMode = AEInputModeFixedAudioFormat;       // 规定频率格式
    _voiceProcessingOnlyForSpeakerAndMicrophone = YES;  //声音处理
    _inputTable = (input_table_t *)calloc(sizeof(input_table_t), 1);
    _inputTable->count = 1;
    _inputTable->entries = (input_entry_t*)calloc(sizeof(input_entry_t), 1);
    
#if TARGET_OS_IPHONE
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationWillEnterForeground:) name:UIApplicationWillEnterForegroundNotification object:nil];   // 监听是否进入前台
#endif
    ...
    // Start housekeeping timer 开启服务定时器
...
- (void)addChannels:(NSArray*)channels toChannelGroup:(AEChannelGroupRef)group {
    ...
    // Configure each channel    配置每个组并且添加混音节点
...
/**
 ...
 @param inRefCon <#inRefCon description#>
 @param inUnit 输入的音频单元
 @param inID 输入id
 @param inScope 输入范围
 @param inElement 输入元素
 */
...
- (BOOL)setup {
    ...
    // Input/output unit description  /*    输入输出的描述       */
...
- (void)configureAudioUnit {
    ...
    if ( _inputEnabled ) {
        ...
        // Register a callback to receive audio  /*   注册一个回调去接收音频数据      */
    ...
    for ( int i = (int)range.location; i < range.location+range.length; i++ ) {
        ...
        if ( channel->type == kChannelTypeChannel ) {
            ...
            if ( hasFilters || channel->audiobusSenderPort ) {
                ...
                // Set input format for callback  设置输入的回调
...
#pragma mark - Callback management  （回调管理）
