 ...
 *
        ...
        AEAudioController已经关闭并重新初始化音频图的通知。他可能是由于一些意想不到的系统错误引起的。
            直接使用图形的对象(如创建音频单元)应该re-initialise音频单元。
 ...
 *
        ...
        一些异步发生错误,比如当用户否认你的应用记录访问。将包含用户信息字典的通知AVAudioControllerErrorKey,NSError。
 ...
 *  Input mode
 *
 *  How to handle incoming audio       如何处理传入的音频
 接收输入的准确,固定音频格式指定初始化音频控制器时,无论输入通道的数量。
 例如,如果您指定一个立体声音频流的描述,和你有一个单一的输入源,mono来源将弥合立体音响。
 ...
 *
        ...
        音频格式将会改变,这取决于可用输入通道的数量。单声道音频源会产生单声道音频;八声道音频来源将产生八声道音频。您可以确定有多少频道提供通过检查的“mNumberBuffers”字段AudioBufferList non-interleaved音频,或“mNumberOfChannels”领域的第一个缓冲区内AudioBufferList交错音频。请注意,这可能会改变在毫无预兆的情况下,当用户插/拔硬件。
 ...
 */
typedef enum {
    AEInputModeFixedAudioFormat,     // 固定格式的音频文件
 ...
 * Render callback
 *      
        这对于音频来讲是必须要的。这叫做从核心音频的实时线程,你不应该等待锁,分配内存,或调用任何objective - c或BSD代码从这个回调。
 ...
 */
@property (nonatomic, readonly) float pan;   // 声道 是只读属性
...
/*!
 * Audio receiver callback              ==          接收音频的回调
 *
 ...
 *  is about to be played by the system.
        ...
        你不应该使用oc的消息机制去操作变量，而应该直接使用成员变量的方式来操作变量
 ...
 *      
            每一个过滤器都会调用 过滤器回调，通过音频过滤生成器来调用的
 ...
 *
 *  Then the audio can be processed as desired. （然后根据需求处理音频）
 *
 ...
 * @param audioController The Audio Controller
 * @param producer  A function pointer to be used to produce input audio  用于创建输入音频的指针函数
 * @param producerToken An opaque pointer to be passed to the producer as the first argument
 * @param time      The time the output audio will be played or the time input audio was received, automatically compensated for hardware latency. 时间戳
 * @param frames    The length of the required audio, in frames  音频帧的长度
 * @param audio     The audio buffer list to write output audio to      音频列表去写入输出音频的缓冲区列表
 * @return A status code     返回一个状态码
 */
 ...
 */
    ...
    /*     这里虽然看起来是属性，但是实际上只有setter 和 getter方法，而没有实例变量         */
@property (nonatomic, readonly) AEAudioFilterCallback filterCallback;
    
@optional
    
/*!
 * Perform setup, to prepare for playback （启动，然后准备去播放）
 *
 *  Filter objects may implement this method to be notified when the object is
 *  being added to the audio controller, or when the audio system is being restored
 *  after a system error.
 ...
 *  is called.
 *
 * @var AEAudioTimingContextInput       （处理正在输入的音频）
 *  Input context: Audio system is about to process some incoming audio (from microphone, etc).
 *
 * @var AEAudioTimingContextOutput
 *  Output context: Audio system is about to render the next buffer for playback.  （去渲染播放下一个缓冲区）
 *
 */
typedef enum {
...
/*!
 @typedef AEAudioControllerOptions  音频引擎的选择条件
...
typedef enum {
    ...
    AEAudioControllerOptionUseVoiceProcessing       = 1 << 2,      // 是否允许使用声音处理
    ...
    AEAudioControllerOptionEnableBluetoothInput     = 1 << 4,  // 蓝牙输入
    /// Whether to allow mixing audio with other apps.
    AEAudioControllerOptionAllowMixingWithOtherApps = 1 << 5,  // 是否允许其他app的混音
    /// Default options
 ...
 *  2. Set required parameters.
        ...
        添加通道,输入接收器,接收器输出,根据需要,时机接收器和过滤器。请注意,所有这些都可以添加/删除操作期间
 ...
 *
 * @param audioDescription    Audio description to use for all audio
 * @param enableInput         Whether to enable audio input from the microphone or another input device   // 是基于I/O来的
...
/*!
 * Add an audio filter to the system output （添加一个音频过滤器，给系统订的输出）
 *
 *  Audio filters are used to process live audio before playback.  （音频过滤器是在播放前处理的实时音频流）
 *
 ...
 *
 *  Audio filters are used to process live audio.（是在处理实时音频的时候使用的）
 *
 * @param filter An object that implements the AEAudioFilter protocol
 */
 ...
 *  mixed together.
 *      输出接收机接收由系统播放的音频。使用此方法添加一个接收器来接收由所有的播放频道混合在一起的音频
 ...
 *  callbacks are called.
        设置为是的静音所有系统输出。注意，即使这是肯定的，播放回调仍将接收音频，为沉默发生后输出接收回调被调用。
 */
@property (nonatomic, assign) BOOL muteOutput;

/*!
 * Access the master output volume
 *      请注意，此值会影响音频引擎的输出；它不修改硬件音量设置。
 ...
 *
   ...
   but works adequately well for speech.   录音的时候消除了 回声和反馈 开启的时候
 *
 ...
 *
 *  Default is YES.  当有耳机或者扬声器的时候，使用内置语音处理系统
 */
