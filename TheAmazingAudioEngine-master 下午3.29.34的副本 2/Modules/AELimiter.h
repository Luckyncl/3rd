 ...
 *
  ...
  smoothly limit output volume to a particular level.
        这个类实现了一个超前的音频限制器。使用它平稳地将输出音量限制在一个特定的水平上
 *
            帧数的延迟  由attack 来标明
 ...
 *
        一旦在持续时间里面  超过该限制 那么就会尽快的失效，回到1.0的增益水平，
 ...
 */
@property (nonatomic, assign) UInt32 decay;   // 衰变
