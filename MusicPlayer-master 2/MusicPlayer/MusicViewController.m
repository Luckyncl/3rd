...
- (void)startTimer
{
   self.timer = [NSTimer scheduledTimerWithTimeInterval:0.5f target:self selector:@selector(showTime) userInfo:nil repeats:YES];
    dlog();
}
...
-(void)showTime{
    ...
    [self displayWord];
    _slider.value = audioPlayer.currentTime / audioPlayer.duration;  // 跟新进度
